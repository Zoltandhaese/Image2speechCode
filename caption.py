...
def caption_image_beam_search(encoder, decoder, image_path, word_map, beam_size=1):
    ...
    """
    #oude versie!! zoals we hem meegekregen hebben
    k = beam_size
...
def caption_old_fashion(encoder, decoder, image, word_map, beam_size=1):
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--model', '-checkpoint_0.pth.tar', help='path to model')
    ...
    word_map_file =  data_path + '/' + 'caps_dic.pickle'
    args = parser.parse_args()
    with open("New_model/imagenames.txt") as f:
        names = f.readlines()
    i = random.randint(0,4000)
    imgname = names[i][:-1] #okay dus het zijn altijd herhalingen van 5 ook dus de names!
    path_to_Image = "New_model/Flicker8k_Dataset/" + imgname +".jpg"
    beam_size= 1
    batch_size = 1
    workers = 0
    ...
    seq, alphas = caption_old_fashion(encoder, decoder, Image1, word_map, beam_size)
    alphas = torch.FloatTensor(alphas)

    # Visualize caption and attention of best sequence
    visualize_att(path_to_Image, seq, alphas, rev_word_map, args.smooth)
